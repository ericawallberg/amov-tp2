...
class GameActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
        ...
        for (i in 0..arrayList.size - 1) {
            val marker = map.addMarker(
                MarkerOptions().position(
                    LatLng(
                        arrayList.get(i).coordenadas.latitude,
                        arrayList.get(i).coordenadas.longitude
                    )
                ).title(arrayList.get(i).nome).snippet(
                    String.format(
                        "%.3f,%.3f",
                        arrayList.get(i).coordenadas.latitude,
                        arrayList.get(i).coordenadas.latitude
    ...
    private fun buildShapes() {
        for (i in 0..arrayList.size - 1)
            arrayCoords.add(arrayList[i].coordenadas)
        val circle = map.addCircle(
            CircleOptions().center(
                LatLng(
                    arrayList.get(0).coordenadas.latitude,
                    arrayList.get(0).coordenadas.longitude
                )
            ).radius(100.0).strokeColor(Color.MAGENTA).fillColor(Color.TRANSPARENT)
        )
        ...
        polygon = map.addPolygon(
            PolygonOptions().addAll(arrayLatLng).strokeColor(Color.DKGRAY).fillColor(Color.BLUE)
        )
    }
    ...
    private fun getApothem(): Double {
        return (arrayDist[0] / (2 * tan((180 / arrayDist.size) * Math.PI / 180)))
    }
